# Taken from RLLib https://github.com/ray-project/ray/blob/master/rllib/examples/two_trainer_workflow.py
...
def custom_training_workflow_ppo_dqn(workers: WorkerSet, config: dict):
    ...
    # def add_a3c_metrics(batch):
    #     print("A3C policy learning on samples from",
    #           batch.policy_batches.keys(), "env steps", batch.env_steps(),
    #           "agent steps", batch.env_steps())
    #     metrics = _get_shared_metrics()
    #     metrics.counters["agent_steps_trained_A3C"] += batch.env_steps()
    #     return batch
    ...
    # # A3C sub-flow.
    # a3c_train_op = r2.for_each(SelectExperiences(["A3C_policy"])) \
    #     .combine(ConcatBatches(
    #         min_batch_size=200)) \
    #     .for_each(add_a3c_metrics) \
    #     .for_each(StandardizeFields(["advantages"])) \
    #     .for_each(TrainOneStep(
    #         workers,
    #         policies=["A3C_policy"],
...
def custom_training_workflow_ppo_a3c(workers: WorkerSet, config: dict):
    local_replay_buffer = LocalReplayBuffer(
        num_shards=1,
        learning_starts=1000,
        buffer_size=50000,
    ...
    # Combined training flow
    train_op = Concurrently(
        [ppo_train_op, a3c_train_op], mode="async", output_indexes=[1])

    return StandardMetricsReporting(train_op, workers, config)

def custom_training_workflow_dqn_a3c(workers: WorkerSet, config: dict):
    local_replay_buffer = LocalReplayBuffer(
        num_shards=1,
        learning_starts=1000,
        buffer_size=50000,
    ...
    # Combined training flow
    train_op = Concurrently(
        [dqn_train_op, a3c_train_op], mode="async", output_indexes=[1])
...
def custom_training_workflow_ppo_ddpg(workers: WorkerSet, config: dict):
    ...
    # Combined training flow
    train_op = Concurrently(
        [ppo_train_op, ddpg_train_op], mode="async", output_indexes=[1])
