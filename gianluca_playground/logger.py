...
def configure(
    dir=None, format_strs=None, experiment_name=None, experiment_subdirectory=None
):
    if dir is None:
        dir = os.getenv("OPENAI_LOGDIR")
    if dir is None:
        curpath = os.getcwd()
        totpath = curpath + "/logs"
        dir = totpath
    if experiment_name is not None:
        dir = f"{dir}/{experiment_name}"
    else:
        dir = osp.join(
            dir,
            datetime.datetime.now().strftime("openai_baselines-%Y-%m-%d-%H-%M-%S-%f"),
        )

    if experiment_subdirectory is not None:
        dir = f"{dir}/{experiment_subdirectory}"
...
def _demo():
    info("hi")
    info("Welcome to the new HarvardAtariHelperAI edits")
